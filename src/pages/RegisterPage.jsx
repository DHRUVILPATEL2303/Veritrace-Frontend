...
import {
  Upload, Fingerprint, Shield, CheckCircle2, FilePlus, AlertTriangle,
  Loader2, ExternalLink, Award, ChevronRight, Bot, Webhook, Check
} from 'lucide-react'
import {
...
export default function RegisterPage() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (showAiRequirement) {
        setError(`Registration Blocked: AI analyzer detected ${Math.round(maxConf * 100)}% probability this media is AI-generated. Please select the correct AI model.`);
      ...
      try {
        ...
        if (checkErr.message.includes('already been registered')) throw checkErr
      }
      ...
      const pinMetaRes = await fetch(`${CORE_BACKEND_API}/api/v1/pin`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(metadataPayload),
      })
      ...
      try {
        localStorage.setItem(`vt_media_${sha256Bytes32.toLowerCase()}`, JSON.stringify({
          sha256: sha256Bytes32, media_ipfs_url: mediaIpfsUrl, media_s3_url: mediaS3Url,
          media_type: hashes.mediaType || 'image', ipfsCid, keyframes: hashes.keyframes || [],
        }))
      } catch {}

      setTxResult({
        hash: receipt.transactionHash, blockNumber: Number(receipt.blockNumber),
        mediaIpfsUrl, mediaS3Url, ipfsCid, sha256: sha256Bytes32,
      })
      setStep(4)
    } catch (err) {
  ...
  return (
    <section className="max-w-[1280px] mx-auto px-5 pt-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-1">Register Content</h1>
        <p className="text-sm text-[var(--color-text-muted)]">Upload a file to fingerprint and register its authenticity on-chain</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-5">
          <Card>
            <CardHeader>
              <CardTitle><Upload size={16} className="text-blue-400" /> File Upload</CardTitle>
            </CardHeader>
            <CardBody>
          ...
          <AnimatePresence>
            {(processing || hashes.sha256) && (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
                <Card>
                  <CardHeader>
                    <CardTitle><Fingerprint size={16} className="text-blue-400" /> Content Fingerprints</CardTitle>
                    {processing && <Badge variant="info">Computing...</Badge>}
                  </CardHeader>
                  <CardBody className="flex flex-col gap-3">
                    {processing ? (
                      <div className="p-3 rounded-lg bg-[var(--color-base-50)] border border-[var(--color-border)]">
                        <div className="text-[11px] font-semibold uppercase tracking-wider text-[var(--color-text-muted)] mb-1.5">SHA-256 CRYPTOGRAPHIC HASH</div>
                        <div className="shimmer h-9 rounded-md w-full" />
                      </div>
                    ) : (
                      <>
                        ...
                        <div className="flex flex-wrap gap-3 text-xs">
                          ...
                          {hashes.aiConfidenceScore !== undefined && hashes.aiConfidenceScore !== null && (
                            <Meta label="AI Score" value={`${Math.round(maxConf * 100)}%`} color={maxConf > 0.75 ? 'text-red-400' : maxConf > 0.3 ? 'text-amber-400' : 'text-emerald-400'} bold />
                          )}
                        </div>

                        {hashes.semanticHash?.length > 0 && <VectorBlock tag="SEM" label="Semantic Embedding" color="from-purple-500 to-indigo-500" data={hashes.semanticHash} />}
                        {hashes.faceHashes?.length > 0 && <VectorBlock tag="FACE" label="ArcFace Biometric" color="from-emerald-500 to-teal-500" data={hashes.faceHashes} multi />}
                        {hashes.audioHashes?.length > 0 && <VectorBlock tag="AUD" label="wav2vec2 Voice" color="from-red-500 to-orange-500" data={hashes.audioHashes} multi />}

                        {hashes.keyframes?.length > 0 && (
                          <div className="border-t border-[var(--color-border)] pt-3">
                            <div className="flex items-center justify-between mb-1.5">
                              <span className="text-xs font-semibold text-[var(--color-text-muted)]">Extracted Keyframes ({hashes.keyframes.length})</span>
                              <button onClick={() => setShowAllKeyframes(!showAllKeyframes)} className="text-xs text-blue-400 hover:text-blue-300">
                                {showAllKeyframes ? 'Show Less' : 'Show All'}
                              </button>
                            </div>
                            <div className="max-h-[220px] overflow-y-auto p-2 rounded-lg bg-[var(--color-base-50)] border border-[var(--color-border)] flex flex-col gap-1 font-mono text-xs">
                              {(showAllKeyframes ? hashes.keyframes : hashes.keyframes.slice(0, 2)).map((kf, i) => {
                                ...
                                return (
                                  <div key={i} className="flex flex-col gap-0.5 pb-1 border-b border-[var(--color-border)] last:border-0">
                                    <div className="flex justify-between">
                                      <span>⏱️ {offset}ms</span>
                                      <span className="text-blue-400">🔑 {phash}</span>
                                    </div>
                                    {kf.semantic_hash?.length > 0 && <span className="text-[var(--color-text-muted)] text-[10px]">🧠 [{kf.semantic_hash.slice(0,3).map(v => typeof v === 'number' ? v.toFixed(3) : v).join(', ')}, …]</span>}
                                  </div>
                                )
                              })}
                            </div>
        ...
        <div className="flex flex-col gap-5">
          <Card>
            <CardHeader>
              <CardTitle><Shield size={16} className="text-blue-400" /> Blockchain Registration</CardTitle>
            </CardHeader>
            <CardBody>
              <AnimatePresence mode="wait">
                    ...
                    {uploadProgress < 100 ? (
                      <>
                        <div className="flex justify-between text-xs font-semibold mb-1.5">
                          <span className="text-[var(--color-text-secondary)]">Uploading Media File...</span>
                          <span className="text-blue-400">{uploadProgress}%</span>
                        </div>
                        <Progress value={uploadProgress} />
                        <div className="text-xs text-[var(--color-text-muted)] mt-2 text-center">Uploading payload to remote Hash Engine</div>
                      </>
                    ) : (
                      <div className="text-center py-4">
                        <Spinner size={40} />
                        <div className="font-semibold mt-3 text-sm">Extracting & Hashing...</div>
                        <div className="text-xs text-[var(--color-text-muted)] mt-1">Generating signatures. This may take up to a minute for larger videos.</div>
                      </div>
                    )}
                  </motion.div>
                )}
                ...
                {step === 2 && !processing && (
                  <motion.div key="ready" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex flex-col gap-4">
                    ...
                    <div>
                      <label className="text-xs font-semibold text-[var(--color-text-muted)] block mb-1.5 flex items-center gap-1">
                        <Bot size={12} /> AI Generator Attribution
                      </label>
                      <Select
                        value={aiCategory}
                        onChange={(e) => {
                          ...
                          setAiCategory(val)
                          if (val === 'None (Authentic Content)') setAiTool('')
                          else if (val !== 'Other (Custom Input)') setAiTool(val)
                        }}
                      >
                        {AI_MODELS.map(m => <option key={m} value={m}>{m}</option>)}
                      </Select>
                      {showAiRequirement && (
                        <div className="text-xs text-red-400 mt-1.5 font-medium flex items-center gap-1">
                          <AlertTriangle size={12} /> AI detected ({Math.round(maxConf * 100)}%). You must declare the AI model.
                        </div>
                      )}
                      {aiCategory === 'Other (Custom Input)' && (
                        <input
                          ...
                          className="w-full mt-2 px-3.5 py-2.5 text-sm rounded-lg bg-[var(--color-base-50)] border border-[var(--color-border)] text-[var(--color-text)] outline-none focus:border-blue-500/50"
                        />
                      )}
                    </div>

                    <label className="flex items-center gap-2 text-xs cursor-pointer text-[var(--color-text-secondary)]">
                      <input type="checkbox" checked={allowAiTraining} onChange={(e) => setAllowAiTraining(e.target.checked)} className="accent-blue-500" />
                      Allow AI models to use this content for training
                    </label>

                    <div>
                      <label className="text-xs font-semibold text-[var(--color-text-muted)] block mb-1.5 flex items-center gap-1">
                        <Webhook size={12} /> Webhook URL (Optional)
                      </label>
                      <input
                        type="url"
                        value={webhookUrl}
                        onChange={(e) => setWebhookUrl(e.target.value)}
                        placeholder="e.g. Discord, Slack webhook"
                        className="w-full px-3.5 py-2.5 text-sm rounded-lg bg-[var(--color-base-50)] border border-[var(--color-border)] text-[var(--color-text)] outline-none focus:border-blue-500/50"
                      />
                    </div>

                    <div className="bg-[var(--color-base-50)] rounded-xl p-4 border border-[var(--color-border)]">
                      <div className="text-[11px] font-semibold uppercase tracking-wider text-[var(--color-text-muted)] mb-2">Transaction Preview</div>
                    ...
                    <Button variant="primary" size="lg" className="w-full" onClick={handleRegister}>
                      <Shield size={18} /> Register on Blockchain
                    </Button>
                  </motion.div>
                )}
                ...
                {step === 3 && signing && (
                  <motion.div key="signing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-8">
                    <Spinner size={40} />
                    <div className="font-semibold mt-3 text-sm">Waiting for confirmation...</div>
                    <div className="text-xs text-[var(--color-text-muted)] mt-1">Please confirm the transaction in your wallet</div>
                  </motion.div>
                )}
                ...
                {step === 4 && txResult && (
                  <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-4">
                    <motion.div
                      ...
                      className="w-14 h-14 rounded-full bg-emerald-400/10 text-emerald-400 flex items-center justify-center mx-auto mb-3"
                    >
                      <CheckCircle2 size={28} />
                    </motion.div>
                    <div className="font-bold text-lg mb-1">Successfully Registered!</div>
                    <div className="text-xs text-[var(--color-text-muted)] mb-4">Your content has been anchored on Arbitrum Sepolia</div>

                    <div className="bg-[var(--color-base-50)] rounded-xl p-4 border border-[var(--color-border)] text-left text-xs mb-4">
                      <TxRow label="Tx Hash">
                        <a href={`${ARBITRUM_SEPOLIA.explorer}/tx/${txResult.hash}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 font-mono">
                      ...
                      {txResult.ipfsCid && <TxRow label="Metadata (IPFS)"><a href={`https://gateway.pinata.cloud/ipfs/${txResult.ipfsCid}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">View ↗</a></TxRow>}
                      {txResult.mediaIpfsUrl && <TxRow label="Media (IPFS)"><a href={txResult.mediaIpfsUrl.replace('ipfs://', 'https://gateway.pinata.cloud/ipfs/')} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">View ↗</a></TxRow>}
                      {txResult.mediaS3Url && <TxRow label="Media (S3)"><a href={txResult.mediaS3Url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">View ↗</a></TxRow>}
                    </div>

                    <div className="flex flex-col gap-2">
                      <Button variant="success" size="sm" onClick={() => downloadCertificate(txResult, address, CORE_BACKEND_API)}>
                        <Award size={16} /> Download PDF Certificate
                      </Button>
                      <div className="flex gap-2">
                        <a href={`${ARBITRUM_SEPOLIA.explorer}/tx/${txResult.hash}`} target="_blank" rel="noopener noreferrer" className="flex-1">
                          <Button variant="outline" size="sm" className="w-full"><ExternalLink size={14} /> Arbiscan</Button>
                        </a>
          ...
          <Card>
            ...
            <CardBody className="text-xs leading-relaxed text-[var(--color-text-secondary)]">
              <div className="flex flex-col gap-3">
                <InfoRow label="On-Chain" color="text-blue-400" items={['SHA-256 hash (bytes32)', 'Wallet address (msg.sender)', 'Block timestamp', 'AI tool attribution']} />
                <InfoRow label="Hash Engine (BoltDB)" color="text-cyan-400" items={['SHA-256 hash', 'Perceptual hash units', 'File content (for verification)', 'Asset metadata']} />
                <InfoRow label="Backend (Postgres/Qdrant)" color="text-emerald-400" items={['Event-sourced metadata', 'pHash vectors (64-dim)', 'Redis exact-match cache']} />
              </div>
            </CardBody>
          </Card>
        </div>
...
function Meta({ label, value, mono, color, bold }) {
  return (
    <div>
      ...
      <span className={`${mono ? 'font-mono text-blue-400' : ''} ${color || ''} ${bold ? 'font-bold' : ''}`}>{value}</span>
    </div>
  )
}

function VectorBlock({ tag, label, color, data, multi }) {
  const isMulti = multi && Array.isArray(data[0])
  const items = isMulti ? data : [data]
  return (
    <div className="bg-[var(--color-base-50)] rounded-lg p-2.5 border border-[var(--color-border)]">
      <div className="flex justify-between items-center mb-1">
        <div className="flex items-center gap-2">
          <span className={`bg-gradient-to-r ${color} text-white rounded px-1.5 py-0.5 text-[10px] font-bold`}>{tag}</span>
          <span className="text-xs font-semibold">{label}</span>
        </div>
        <Badge variant="info" className="text-[10px]">{Array.isArray(data[0]) ? data.length : 1} detected</Badge>
      </div>
      <div className="text-[10px] font-mono text-[var(--color-text-muted)]">
        {items.map((item, i) => (
          <div key={i}>[{item.slice(0, 6).map(v => typeof v === 'number' ? v.toFixed(4) : v).join(', ')}{item.length > 6 ? ', …' : ''}]</div>
        ))}
      </div>
    </div>
  )
...
function TxRow({ label, value, accent, mono, children }) {
  return (
    <div className="flex justify-between items-center mb-1">
      <span className="text-[var(--color-text-muted)]">{label}</span>
      {children || <span className={`${accent ? 'text-blue-400' : ''} ${mono ? 'font-mono' : ''}`}>{value}</span>}
    </div>
  )
}
...
function InfoRow({ label, items, color }) {
  return (
    <div>
      ...
      <ul className="pl-4 m-0 list-disc list-inside">
        {items.map((item, i) => <li key={i} className="text-[var(--color-text-secondary)]">{item}</li>)}
      </ul>
    </div>
  )
}
